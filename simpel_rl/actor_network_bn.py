...
model_dir_actor = os.path.join(os.path.split(os.path.realpath(__file__))[0], 'model','actor')
...
class ActorNetwork:
	...
	def load_network(self):
		...
		checkpoint = tf.train.get_checkpoint_state(model_dir_actor)
	...
	def save_network(self,time_step):
		...
		self.saver.save(self.sess, os.path.join(model_dir_actor, 'actor-network'), global_step=time_step)
